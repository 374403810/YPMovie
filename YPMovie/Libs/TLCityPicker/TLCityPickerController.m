...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.backgroundColor=YPSYSTEMCOLOR(whiteColor);
    //设置返回按钮
    UIButton * back = [UIButton buttonWithType:UIButtonTypeCustom];
    back.frame=CGRectMake(20, 0, 50, 50);
    [back setTitle:@"返回" forState:UIControlStateNormal];
    [back setTitleColor:YPSYSTEMCOLOR(blackColor) forState:UIControlStateNormal];
    [back addTarget:self action:@selector(cancelButtonDown) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *cancelBarButton = [[UIBarButtonItem alloc] initWithCustomView:back];
